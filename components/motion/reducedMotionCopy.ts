...
/* The 50 languages covering the large majority of the site's likely global
   ...
   Body copy stays short on purpose: what changed, why, where to fix it,
   done. An earlier draft spelled out everything the static mode turns off
   (no 3D scene, no mascot, no scroll animation) and that was the wrong
   amount of detail for something people are meant to read in passing. */
export type Lang =
  | "en"
  | "zh-Hans"
  | "zh-Hant"
...
export const COPY: Record<Lang, Entry> = {
  en: {
    body: (os) =>
      `This device has reduced motion turned on, so the page is showing its static version. If that wasn't on purpose, turn it back on in ${
        os === "windows"
  ...
  "zh-Hans": {
    body: (os) =>
      `此设备已开启"减弱动态效果"，因此页面显示的是静态版本。如果这并非你的本意，可以在${
        os === "windows"
  ...
  "zh-Hant": {
    body: (os) =>
      `此裝置已開啟「減少動態效果」，因此頁面顯示的是靜態版本。如果這並非你的本意，可以在${
        os === "windows"
  ...
  hi: {
    body: (os) =>
      `इस डिवाइस में "मोशन कम करें" चालू है, इसलिए यह पेज अपना स्थिर रूप दिखा रहा है। अगर यह जानबूझकर नहीं किया गया था, तो इसे ${
        os === "windows"
      ...
      } वापस चालू करें, फिर पेज को रीलोड करें।`,
    dismiss: "यह सूचना बंद करें",
  },
  es: {
    body: (os) =>
      `Este dispositivo tiene el movimiento reducido activado, así que esta página muestra su versión estática. Si no fue intencional, puedes activarlo de nuevo en ${
        os === "windows"
  ...
  fr: {
    body: (os) =>
      `Cet appareil a la réduction des animations activée, donc cette page affiche sa version statique. Si ce n'était pas voulu, vous pouvez la réactiver dans ${
        os === "windows"
  ...
  ar: {
    body: (os) =>
      `تم تفعيل خاصية "تقليل الحركة" على هذا الجهاز، لذلك تعرض هذه الصفحة نسختها الثابتة. إذا لم يكن هذا مقصودًا، يمكنك إعادة تفعيلها من ${
        os === "windows"
  ...
  bn: {
    body: (os) =>
      `এই ডিভাইসে "রিডিউসড মোশন" চালু আছে, তাই এই পেজটি তার স্থির সংস্করণ দেখাচ্ছে। যদি এটি ইচ্ছাকৃত না হয়ে থাকে, তাহলে ${
        os === "windows"
      ...
      } এটি আবার চালু করতে পারেন, তারপর পেজটি রিলোড করুন।`,
    dismiss: "এই বার্তাটি বন্ধ করুন",
  },
  pt: {
    body: (os) =>
      `Este dispositivo está com a redução de movimento ativada, por isso esta página mostra a sua versão estática. Se isso não foi intencional, você pode reativá-la em ${
        os === "windows"
      ...
      }, e depois recarregar a página.`,
    dismiss: "Fechar este aviso",
  },
  ru: {
    body: (os) =>
      `На этом устройстве включено уменьшение движения, поэтому страница показывает статичную версию. Если это произошло случайно, включить обратно можно ${
        os === "windows"
      ...
      }, а затем перезагрузить страницу.`,
    dismiss: "Закрыть это уведомление",
  },
  ur: {
    body: (os) =>
      `اس ڈیوائس میں "حرکت میں کمی" فعال ہے، اس لیے یہ صفحہ اپنا ساکت ورژن دکھا رہا ہے۔ اگر یہ ارادی نہیں تھا تو آپ اسے ${
        os === "windows"
      ...
      } دوبارہ فعال کر سکتے ہیں، پھر صفحے کو ری لوڈ کریں۔`,
    dismiss: "یہ اطلاع بند کریں",
  },
  id: {
    body: (os) =>
      `Perangkat ini mengaktifkan pengurangan gerakan, jadi halaman ini menampilkan versi statisnya. Jika ini tidak disengaja, kamu bisa mengaktifkannya lagi di ${
        os === "windows"
  ...
  de: {
    body: (os) =>
      `Auf diesem Gerät ist „Bewegung reduzieren" aktiviert, deshalb zeigt diese Seite ihre statische Version. Falls das nicht beabsichtigt war, kannst du es ${
        os === "windows"
      ...
      } wieder aktivieren und die Seite danach neu laden.`,
    dismiss: "Diesen Hinweis schließen",
  },
  ja: {
    body: (os) =>
      `このデバイスは「視差効果を減らす」がオンになっているため、このページは静的なバージョンを表示しています。意図的でない場合は、${
        os === "windows"
      ...
      }再度オンにしてから、ページを再読み込みしてください。`,
    dismiss: "この通知を閉じる",
  },
  mr: {
    body: (os) =>
      `या डिव्हाइसवर "मोशन कमी करा" सुरू आहे, म्हणून हे पेज त्याची स्थिर आवृत्ती दाखवत आहे. हे हेतुपुरस्सर नसेल, तर तुम्ही ते ${
        os === "windows"
      ...
      } पुन्हा सुरू करू शकता, नंतर पेज रीलोड करा.`,
    dismiss: "ही सूचना बंद करा",
  },
  te: {
    body: (os) =>
      `ఈ పరికరంలో "మోషన్ తగ్గించు" ఆన్‌లో ఉంది, కాబట్టి ఈ పేజీ దాని స్థిర వెర్షన్‌ను చూపిస్తోంది. ఇది ఉద్దేశపూర్వకంగా చేయకపోతే, దీన్ని ${
        os === "windows"
      ...
      } మళ్లీ ఆన్ చేయవచ్చు, తర్వాత పేజీని రీలోడ్ చేయండి.`,
    dismiss: "ఈ నోటీసును మూసివేయండి",
  },
  tr: {
    body: (os) =>
      `Bu cihazda hareketi azalt açık olduğu için bu sayfa sabit sürümünü gösteriyor. Bu kasıtlı değilse, ${
        os === "windows"
      ...
      } yeniden açabilir, ardından sayfayı yenileyebilirsiniz.`,
    dismiss: "Bu bildirimi kapat",
  },
  ta: {
    body: (os) =>
      `இந்தச் சாதனத்தில் "இயக்கத்தைக் குறை" இயக்கத்தில் உள்ளது, எனவே இந்தப் பக்கம் அதன் நிலையான பதிப்பைக் காட்டுகிறது. இது வேண்டுமென்றே செய்யப்படவில்லை என்றால், இதை ${
        os === "windows"
      ...
      } மீண்டும் இயக்கலாம், பிறகு பக்கத்தை மீண்டும் ஏற்றவும்.`,
    dismiss: "இந்த அறிவிப்பை மூடு",
  },
  vi: {
    body: (os) =>
      `Thiết bị này đang bật chế độ giảm chuyển động, nên trang này hiển thị phiên bản tĩnh của nó. Nếu bạn không cố ý bật chế độ này, bạn có thể bật lại nó trong ${
        os === "windows"
  ...
  ko: {
    body: (os) =>
      `이 기기는 "동작 줄이기"가 켜져 있어서, 이 페이지는 정적인 버전을 보여주고 있습니다. 의도한 것이 아니라면 ${
        os === "windows"
          ? `Windows의 접근성 설정에서 "애니메이션 효과"와 비슷한 항목을`
          : os === "mac"
            ? `Mac의 시스템 설정 접근성에서 "동작 줄이기"와 비슷한 항목을`
            : `기기의 접근성 또는 디스플레이 설정에서 "동작 줄이기"와 비슷한 항목을`
      } 다시 켠 다음, 페이지를 새로고침하면 됩니다.`,
    dismiss: "이 알림 닫기",
  },
  it: {
    body: (os) =>
      `Questo dispositivo ha la riduzione del movimento attivata, quindi questa pagina mostra la sua versione statica. Se non era intenzionale, puoi riattivarla ${
        os === "windows"
  ...
  pl: {
    body: (os) =>
      `To urządzenie ma włączone ograniczenie ruchu, więc ta strona pokazuje swoją statyczną wersję. Jeśli to nie było zamierzone, możesz włączyć to ponownie ${
        os === "windows"
  ...
  uk: {
    body: (os) =>
      `На цьому пристрої увімкнено зменшення руху, тому ця сторінка показує статичну версію. Якщо це сталося не навмисно, увімкнути назад можна ${
        os === "windows"
      ...
      }, а потім перезавантажити сторінку.`,
    dismiss: "Закрити це повідомлення",
  },
  ro: {
    body: (os) =>
      `Acest dispozitiv are mișcarea redusă activată, așa că această pagină își arată versiunea statică. Dacă nu a fost intenționat, o poți reactiva din ${
        os === "windows"
  ...
  nl: {
    body: (os) =>
      `Dit apparaat heeft beweging verminderen ingeschakeld, dus deze pagina laat zijn statische versie zien. Als dat niet de bedoeling was, kun je het weer aanzetten ${
        os === "windows"
      ...
      }, en deze pagina daarna herladen.`,
    dismiss: "Deze melding sluiten",
  },
  th: {
    body: (os) =>
      `อุปกรณ์นี้เปิดใช้งาน "ลดการเคลื่อนไหว" อยู่ หน้านี้จึงแสดงเวอร์ชันนิ่งของมัน หากไม่ได้ตั้งใจ คุณสามารถเปิดกลับมาได้ที่${
        os === "windows"
  ...
  gu: {
    body: (os) =>
      `આ ડિવાઇસમાં "મોશન ઘટાડો" ચાલુ છે, તેથી આ પેજ તેની સ્થિર આવૃત્તિ બતાવી રહ્યું છે. જો આ ઇરાદાપૂર્વક ન હોય, તો તમે તેને ${
        os === "windows"
      ...
      } ફરીથી ચાલુ કરી શકો છો, પછી પેજ ફરીથી લોડ કરો.`,
    dismiss: "આ સૂચના બંધ કરો",
  },
  fa: {
    body: (os) =>
      `در این دستگاه گزینه "کاهش حرکت" روشن است، به همین دلیل این صفحه نسخه ثابت خود را نشان می‌دهد. اگر این کار عمدی نبوده، می‌توانید آن را ${
        os === "windows"
      ...
      } دوباره روشن کنید، سپس صفحه را دوباره بارگذاری کنید.`,
    dismiss: "بستن این اعلان",
  },
  pa: {
    body: (os) =>
      `ਇਸ ਡਿਵਾਈਸ ਵਿੱਚ "ਮੋਸ਼ਨ ਘਟਾਓ" ਚਾਲੂ ਹੈ, ਇਸ ਲਈ ਇਹ ਪੇਜ ਆਪਣਾ ਸਥਿਰ ਰੂਪ ਦਿਖਾ ਰਿਹਾ ਹੈ। ਜੇ ਇਹ ਜਾਣਬੁੱਝ ਕੇ ਨਹੀਂ ਸੀ, ਤਾਂ ਤੁਸੀਂ ਇਸਨੂੰ ${
        os === "windows"
      ...
      } ਦੁਬਾਰਾ ਚਾਲੂ ਕਰ ਸਕਦੇ ਹੋ, ਫਿਰ ਪੇਜ ਨੂੰ ਰੀਲੋਡ ਕਰੋ।`,
    dismiss: "ਇਹ ਸੂਚਨਾ ਬੰਦ ਕਰੋ",
  },
  sw: {
    body: (os) =>
      `Kifaa hiki kimewasha kupunguza mwendo, hivyo ukurasa huu unaonyesha toleo lake tuli. Kama hukukusudia hili, unaweza kukiwasha tena kwenye ${
        os === "windows"
      ...
      }, kisha upakie upya ukurasa huu.`,
    dismiss: "Funga arifa hii",
  },
  ha: {
    body: (os) =>
      `Na'urar nan tana da rage motsi a kunne, don haka wannan shafin yana nuna sigar sa wadda ba ta motsi. Idan ba a yi wannan da gangan ba, za ka iya sake kunna shi a ${
        os === "windows"
      ...
      }, sannan ka sake loda wannan shafin.`,
    dismiss: "Rufe wannan sanarwa",
  },
  kn: {
    body: (os) =>
      `ಈ ಸಾಧನದಲ್ಲಿ "ಚಲನೆ ಕಡಿಮೆ ಮಾಡು" ಆನ್ ಆಗಿದೆ, ಆದ್ದರಿಂದ ಈ ಪುಟವು ಅದರ ಸ್ಥಿರ ಆವೃತ್ತಿಯನ್ನು ತೋರಿಸುತ್ತಿದೆ. ಇದು ಉದ್ದೇಶಪೂರ್ವಕವಾಗಿ ಆಗಿಲ್ಲದಿದ್ದರೆ, ಇದನ್ನು ${
        os === "windows"
      ...
      } ಮತ್ತೆ ಆನ್ ಮಾಡಬಹುದು, ನಂತರ ಪುಟವನ್ನು ಮರುಲೋಡ್ ಮಾಡಿ.`,
    dismiss: "ಈ ಸೂಚನೆಯನ್ನು ಮುಚ್ಚಿ",
  },
  ml: {
    body: (os) =>
      `ഈ ഉപകരണത്തിൽ "ചലനം കുറയ്ക്കുക" ഓണാണ്, അതിനാൽ ഈ പേജ് അതിന്റെ സ്ഥിര പതിപ്പ് കാണിക്കുന്നു. ഇത് മനഃപൂർവ്വമല്ലെങ്കിൽ, ഇത് ${
        os === "windows"
      ...
      } വീണ്ടും ഓണാക്കാം, എന്നിട്ട് പേജ് വീണ്ടും ലോഡ് ചെയ്യുക.`,
    dismiss: "ഈ അറിയിപ്പ് അടയ്ക്കുക",
  },
  my: {
    body: (os) =>
      `ဒီစက်ပစ္စည်းမှာ "လှုပ်ရှားမှုလျှော့ချရန်" ဖွင့်ထားလို့၊ ဒီစာမျက်နှာက ၎င်း၏ တည်ငြိမ်တဲ့ ဗားရှင်းကို ပြသနေပါတယ်။ ဒါကို တမင်မလုပ်ခဲ့ဘူးဆိုရင်၊ ${
        os === "windows"
  ...
  am: {
    body: (os) =>
      `ይህ መሣሪያ እንቅስቃሴን መቀነስ በርቷል፣ ስለዚህ ይህ ገጽ የማይንቀሳቀስ ስሪቱን እያሳየ ነው። ይህ ሆን ተብሎ ካልነበረ፣ ${
        os === "windows"
      ...
      } እንደገና ማብራት ይችላሉ፣ ከዚያ ገጹን እንደገና ይጫኑት።`,
    dismiss: "ይህን ማሳወቂያ ዝጋ",
  },
  az: {
    body: (os) =>
      `Bu cihazda hərəkəti azalt aktivdir, ona görə bu səhifə statik versiyasını göstərir. Bu qəsdən edilməyibsə, onu ${
        os === "windows"
      ...
      } yenidən aça bilərsiniz, sonra səhifəni yeniləyin.`,
    dismiss: "Bu bildirişi bağla",
  },
  or: {
    body: (os) =>
      `ଏହି ଡିଭାଇସ୍‌ରେ "ଗତି କମାନ୍ତୁ" ଅନ୍ ଅଛି, ତେଣୁ ଏହି ପୃଷ୍ଠା ତାହାର ସ୍ଥିର ସଂସ୍କରଣ ଦେଖାଉଛି। ଏହା ଇଚ୍ଛାକୃତ ନ ଥିଲେ, ଆପଣ ଏହାକୁ ${
        os === "windows"
      ...
      } ପୁଣି ଅନ୍ କରିପାରିବେ, ତା'ପରେ ପୃଷ୍ଠାକୁ ପୁଣି ଲୋଡ୍ କରନ୍ତୁ।`,
    dismiss: "ଏହି ବିଜ୍ଞପ୍ତି ବନ୍ଦ କରନ୍ତୁ",
  },
  he: {
    body: (os) =>
      `במכשיר הזה מופעלת הפחתת תנועה, ולכן הדף הזה מציג את הגרסה הסטטית שלו. אם זה לא היה מכוון, אפשר להפעיל את זה מחדש ${
        os === "windows"
  ...
  el: {
    body: (os) =>
      `Αυτή η συσκευή έχει ενεργοποιημένη τη μείωση κίνησης, γι' αυτό η σελίδα εμφανίζει τη στατική της έκδοση. Αν αυτό δεν ήταν σκόπιμο, μπορείτε να το ενεργοποιήσετε ξανά ${
        os === "windows"
      ...
      }, και μετά να ανανεώσετε τη σελίδα.`,
    dismiss: "Κλείσιμο αυτής της ειδοποίησης",
  },
  cs: {
    body: (os) =>
      `Toto zařízení má zapnuté omezení pohybu, proto tato stránka zobrazuje svou statickou verzi. Pokud to nebylo záměrné, můžete to znovu zapnout ${
        os === "windows"
      ...
      }, a poté tuto stránku znovu načíst.`,
    dismiss: "Zavřít toto oznámení",
  },
  sv: {
    body: (os) =>
      `Den här enheten har minska rörelse aktiverat, så den här sidan visar sin statiska version. Om det inte var avsiktligt kan du slå på det igen ${
        os === "windows"
      ...
      }, och sedan ladda om sidan.`,
    dismiss: "Stäng den här notisen",
  },
  hu: {
    body: (os) =>
      `Ezen az eszközön be van kapcsolva a mozgás csökkentése, ezért ez az oldal a statikus verzióját mutatja. Ha ez nem szándékosan történt, újra bekapcsolhatod ${
        os === "windows"
    ...
    body: (os) =>
      `Tässä laitteessa on liikkeen vähentäminen käytössä, joten tämä sivu näyttää staattisen versionsa. Jos tämä ei ollut tarkoituksellista, voit ottaa sen uudelleen käyttöön ${
        os === "windows"
      ...
      }, ja ladata sivun sitten uudelleen.`,
    dismiss: "Sulje tämä ilmoitus",
  },
  da: {
    body: (os) =>
      `Denne enhed har reducer bevægelse slået til, så denne side viser sin statiske version. Hvis det ikke var med vilje, kan du slå det til igen ${
        os === "windows"
      ...
      }, og derefter genindlæse siden.`,
    dismiss: "Luk denne meddelelse",
  },
  no: {
    body: (os) =>
      `Denne enheten har redusert bevegelse slått på, så denne siden viser sin statiske versjon. Hvis dette ikke var meningen, kan du slå det på igjen ${
        os === "windows"
      ...
      }, og deretter laste siden på nytt.`,
    dismiss: "Lukk dette varselet",
  },
  sk: {
    body: (os) =>
      `Toto zariadenie má zapnuté obmedzenie pohybu, preto táto stránka zobrazuje svoju statickú verziu. Ak to nebolo zámerné, môžete to znova zapnúť ${
        os === "windows"
      ...
      }, a potom túto stránku znova načítať.`,
    dismiss: "Zavrieť toto oznámenie",
  },
  bg: {
    body: (os) =>
      `Това устройство има включено намаляване на движението, затова тази страница показва статичната си версия. Ако това не е било нарочно, можете да го включите отново ${
        os === "windows"
      ...
      }, а след това презаредете страницата.`,
    dismiss: "Затвори това известие",
  },
  hr: {
    body: (os) =>
      `Ovaj uređaj ima uključeno smanjenje pokreta, pa ova stranica prikazuje svoju statičnu verziju. Ako to nije bilo namjerno, možete to ponovno uključiti ${
        os === "windows"
      ...
      }, a zatim ponovno učitati stranicu.`,
    dismiss: "Zatvori ovu obavijest",
  },
  ms: {
    body: (os) =>
      `Peranti ini mempunyai kurangkan gerakan diaktifkan, jadi halaman ini menunjukkan versi statiknya. Jika ini tidak disengajakan, anda boleh menghidupkannya semula ${
        os === "windows"
      ...
      }, kemudian muat semula halaman ini.`,
    dismiss: "Tutup pemberitahuan ini",
  },
  fil: {
    body: (os) =>
      `Naka-on ang reduced motion sa device na ito, kaya static na bersyon ang ipinapakita ng pahinang ito. Kung hindi ito sinasadya, puwede mo itong i-on ulit sa ${
        os === "windows"
      ...
      }, pagkatapos i-reload ang pahina.`,
